...
type Session interface {
	...
	WrapKey(mech *pkcs11.Mechanism, wrappingKey, key Object) ([]byte, error)

	UnwrapKey(mech *pkcs11.Mechanism, wrappingKey Object, encryptedKey []byte, template []*pkcs11.Attribute) (*Object, error)
...
func (s *sessionImpl) WrapKey(mech *pkcs11.Mechanism, wrappingKey, key Object) ([]byte, error) {
	s.Lock()
	defer s.Unlock()
	return s.ctx.WrapKey(s.handle, []*pkcs11.Mechanism{mech}, wrappingKey.objectHandle, key.objectHandle)
}

func (s *sessionImpl) UnwrapKey(mech *pkcs11.Mechanism, wrappingKey Object, encryptedKey []byte, template []*pkcs11.Attribute) (*Object, error) {
	s.Lock()
	defer s.Unlock()
	oh, err := s.ctx.UnwrapKey(s.handle, []*pkcs11.Mechanism{mech}, wrappingKey.objectHandle, encryptedKey, template)
	if err != nil {
		return nil, err
	}
	return &Object{
...
func (s *sessionImpl) GenerateBIP32MasterKeyPair(seed SecretKey, label string) (*KeyPair, error) {
	...
	privateTemplate := []*pkcs11.Attribute{
		pkcs11.NewAttribute(pkcs11.CKA_TOKEN, true),
		pkcs11.NewAttribute(pkcs11.CKA_KEY_TYPE, pkcs11.CKK_BIP32),
		pkcs11.NewAttribute(pkcs11.CKA_BIP32_VERSION_BYTES, uint(pkcs11.CKG_BIP32_VERSION_MAINNET_PRIV)),
		pkcs11.NewAttribute(pkcs11.CKA_PRIVATE, true),
		pkcs11.NewAttribute(pkcs11.CKA_SENSITIVE, true),
...
func (s *sessionImpl) DeriveChildKeyPair(masterPrivate PrivateKey, path []uint32) (*KeyPair, error) {
	...
	pubHandle, privHandle, _, err := s.ctx.DeriveBIP32ChildKeys(s.handle, masterPrivate.objectHandle, publicTemplate, privateTemplate, path)
	if err != nil {
		return nil, err
	}
	return &KeyPair{
